...
function Home() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center p-4 font-sans antialiased">
            <div className="bg-white rounded-xl shadow-2xl p-8 md:p-10 w-full max-w-2xl transform transition-all duration-300 hover:scale-[1.01] focus-within:scale-[1.01]">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-teal-500">
            Fidgetech AI Q&A Bot
                ...
                <p className="text-center text-gray-600 mb-8 max-w-md mx-auto">
                    Ask questions about technical documentation. The AI will provide answers using Retrieval Augmented Generation (RAG).
                </p>
                        ...
                        {response.retrieved_chunks && response.retrieved_chunks.length > 0 && (
                            <div className="mt-6">
                                <h3 className="text-xl font-semibold text-gray-700 mb-3 flex items-center">
                                    <svg className="w-5 h-5 mr-2 text-blue-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L14.414 5A2 2 0 0115 6.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 0v12h7V6.414L10.586 4H6z" clipRule="evenodd"></path></svg>
                                    Sources Used:
                                </h3>
                                <ul className="space-y-3">
                                    {response.retrieved_chunks.map((chunk, index) => (
                                        <li key={index} className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                                            <p className="font-medium text-blue-800 text-sm mb-1">Snippet from: {response.source_titles && response.source_titles[index] ? response.source_titles[index] : 'Unknown Source'}</p>
                                            <p className="text-gray-700 text-sm italic">"{chunk.substring(0, 150)}..."</p> {/* Displaying a truncated snippet */}
                                        </li>
                                    ))}
                                </ul>
                                <p className="text-gray-500 text-xs mt-2">
                                    (Students will enhance this section to show full chunks or better formatted sources.)
                                </p>
                            </div>
                        )}
                    </div>
